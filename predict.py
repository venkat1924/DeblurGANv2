...
class Predictor:
    def __init__(self, weights_path: str, model_name: str = ''):
        ...
        self.device = torch.device('cpu')  # Ensure everything runs on CPU
    ...
    def __call__(self, img: np.ndarray, mask: Optional[np.ndarray], ignore_mask=True) -> np.ndarray:
        ...
        with torch.no_grad():
            inputs = [img.to(self.device)]  # Run on CPU
            if not ignore_mask:
...
def process_video(pairs, predictor, output_dir):
    for video_filepath, mask in tqdm(pairs):
        ...
        tqdm.write(f'Processing {video_filepath} to {output_filepath}, {fps}fps, resolution: {width}x{height}')
